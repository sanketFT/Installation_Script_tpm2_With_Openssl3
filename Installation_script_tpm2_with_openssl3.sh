...
sudo ldconfig
cd ..
git clone https://github.com/openssl/openssl $PWD/openssl/
cd $PWD/openssl/
./Configure
make
make test
...
sudo ldconfig
cd ..
git clone https://github.com/tpm2-software/tpm2-openssl $PWD/tpm2-openssl/
cd $PWD/tpm2-openssl/
git checkout v1.1.1
cd ..
sudo cp -r $PWD/autoconf-archive-2022.02.11/m4 $PWD/tpm2-openssl/
cd $PWD/tpm2-openssl/
./bootstrap 
./configure 
make -j$(nproc) 
